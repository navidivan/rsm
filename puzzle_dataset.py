...
class PuzzleDataset(IterableDataset):
    ...
    def _iter_test(self, worker_id=0, num_workers=1):
        # Stride = Total Step Size across all workers
        # Each worker processes: i, i + stride, i + 2*stride...
        stride = num_workers * self.config.global_batch_size

        for set_i, (set_name, dataset) in enumerate(self._data.items()):  # type: ignore
            total_examples = len(dataset["inputs"])

            # Start index offset by worker_id
            start_index = worker_id * self.config.global_batch_size
            ...
            while start_index < total_examples:
                ...
                puzzle_indices = []
                if local_start < local_end: # Check if there is data for this rank
                ...
                # Advance by the total stride of all workers
                start_index += stride

    def _iter_train(self, worker_id=0, num_workers=1):
        for set_name, dataset in self._data.items():  # type: ignore
            # Increase epoch count
            self._iters += 1

            # Seed logic:
            # We want each worker to have a unique stream.
            # We shift the seed by the worker_id to ensure non-overlapping random streams.
            worker_seed = self.config.seed + self._iters + (worker_id * 10000)
    ...
    def __iter__(self):
        ...
        if self.config.test_set_mode:
            yield from self._iter_test(worker_id=worker_id, num_workers=num_workers)
        else:
            yield from self._iter_train(worker_id=worker_id, num_workers=num_workers)
